...
system_state = {"started": False, "starting": False}
    ...
    for app_name, script_path in STREAMLIT_SCRIPTS.items():
        ...
        if os.path.exists(script_path):
            success, message = start_streamlit_app(
                app_name, script_path, processes[app_name]["port"]
            )
...
def read_process_output(process, app_name):
    ...
    while True:
        try:
                ...
                if remaining_output:
                    ...
                    for line in lines:
                        ...
                        if line:
                            ...
                            socketio.emit(
                                "console_output",
                                {"app": app_name, "line": formatted_line},
                            )
                break
            ...
            if sys.platform == "win32":
                ...
                if output:
                    ...
                    if line:
                        ...
                        # 发送到前端
                        socketio.emit(
                            "console_output", {"app": app_name, "line": formatted_line}
                        )
                else:
                    # 没有输出时短暂休眠
                    time.sleep(0.1)
            ...
            write_log_to_file(
                app_name, f"[{datetime.now().strftime('%H:%M:%S')}] {error_msg}"
            )
            break
...
def start_streamlit_app(app_name, script_path, port):
    ...
    try:
        ...
        output_thread = threading.Thread(
            target=read_process_output, args=(process, app_name), daemon=True
        )
        output_thread.start()
            ...
            if info["process"].poll() is None:
                ...
                try:
                    response = requests.get(
                        f"http://localhost:{info['port']}", timeout=2
                    )
...
def get_output(app_name):
    ...
    if app_name == "forum":
        try:
            ...
            return jsonify(
                {"success": False, "message": f"读取forum日志失败: {str(e)}"}
            )
...
def test_log(app_name):
    ...
    # 写入测试消息
    test_msg = (
        f"[{datetime.now().strftime('%H:%M:%S')}] 测试日志消息 - {datetime.now()}"
    )
...
def search():
    ...
    check_app_status()
    running_apps = [
        name for name, info in processes.items() if info["status"] == "running"
    ]
...
def get_engine_config():
    """获取Engine配置。"""
    try:
        # 从 .env 文件读取配置，如果不存在则返回默认值
        project_root = Path(__file__).resolve().parent
        cwd_env = Path.cwd() / ".env"
        env_file_path = cwd_env if cwd_env.exists() else (project_root / ".env")

        config = {"insight": True, "media": True, "query": True}  # 默认配置

        if env_file_path.exists():
            env_content = env_file_path.read_text(encoding="utf-8")
            for line in env_content.splitlines():
                line = line.strip()
                if line.startswith("ENABLED_ENGINES="):
...
def update_engine_config():
    ...
    try:
        ...
        if env_file_path.exists():
            ...
            for i, line in enumerate(env_lines):
                if line.strip().startswith("ENABLED_ENGINES="):
                    env_lines[i] = f"ENABLED_ENGINES={config_str}"
                    config_found = True
                    break
        ...
        if not config_found:
            ...
            config_str = json.dumps(config)
            env_lines.append(f"ENABLED_ENGINES={config_str}")
...
def get_system_status():
    ...
    return jsonify(
        {"success": True, "started": state["started"], "starting": state["starting"]}
    )
