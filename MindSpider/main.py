...
class MindSpider:
    ...
    def run_broad_topic_extraction(
        self, extract_date: date = None, keywords_count: int = 100
    ) -> bool:
        ...
        try:
            cmd = [sys.executable, "main.py", "--keywords", str(keywords_count)]

            logger.info(f"执行命令: {' '.join(cmd)}")

            result = subprocess.run(
                cmd, cwd=self.broad_topic_path, timeout=1800  # 30分钟超时
            )
            ...
            if result.returncode == 0:
                ...
                logger.error(
                    f"BroadTopicExtraction模块执行失败，返回码: {result.returncode}"
                )
                return False
    ...
    def run_deep_sentiment_crawling(
        self,
        ...
        try:
            ...
            cmd.extend(
                ["--max-keywords", str(max_keywords), "--max-notes", str(max_notes)]
            )
            ...
            if result.returncode == 0:
                ...
                logger.error(
                    f"DeepSentimentCrawling模块执行失败，返回码: {result.returncode}"
                )
                return False
    ...
    def show_status(self):
        ...
        logger.info(
            f"BroadTopicExtraction模块: {'存在' if broad_topic_exists else '缺失'}"
        )
        logger.info(
            f"DeepSentimentCrawling模块: {'存在' if deep_sentiment_exists else '缺失'}"
        )
...
def main():
    ...
    parser.add_argument(
        "--deep-sentiment", action="store_true", help="只运行情感爬取模块"
    )
    ...
    parser.add_argument(
        "--keywords-count", type=int, default=100, help="话题提取的关键词数量"
    )
    parser.add_argument(
        "--max-keywords", type=int, default=50, help="每个平台最大关键词数量"
    )
    parser.add_argument(
        "--max-notes", type=int, default=50, help="每个关键词最大爬取内容数量"
    )
