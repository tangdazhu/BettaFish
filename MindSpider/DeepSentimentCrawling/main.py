...
class DeepSentimentCrawling:
    ...
    def run_daily_crawling(
        ...
        print(
            f"   总爬取任务: {len(keywords)} × {len(platforms)} = {len(keywords) * len(platforms)}"
        )
        ...
        print(
            f"   成功任务: {crawl_results['successful_tasks']}/{crawl_results['total_tasks']}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="DeepSentimentCrawling - 基于话题的深度情感爬取"
    )
    ...
    # 爬取参数
    parser.add_argument(
        "--max-keywords", type=int, default=50, help="每个平台最大关键词数量 (默认: 50)"
    )
    parser.add_argument(
        "--max-notes", type=int, default=50, help="每个平台最大爬取内容数量 (默认: 50)"
    )
    parser.add_argument(
        "--login-type",
        type=str,
    ...
    parser.add_argument(
        "--days", type=int, default=7, help="查看最近几天的话题 (默认: 7)"
    )
    ...
    try:
        ...
        # 单平台爬取
        if args.platform:
            logger.info(f"开始爬取平台: {args.platform}")
            ...
            if result["success"]:
                logger.info(f"{args.platform} 爬取成功！")
                print(f"\n{args.platform} 爬取成功！")
            else:
                logger.error(
                    f"{args.platform} 爬取失败: {result.get('error', '未知错误')}"
                )
        ...
        logger.info(f"开始多平台爬取: {platforms}")
        result = crawler.run_daily_crawling(
            target_date, platforms, args.max_keywords, args.max_notes, args.login_type
        )

        if result["success"]:
            logger.info("多平台爬取任务完成！")
            print(f"\n多平台爬取任务完成！")
        else:
            logger.error(f"多平台爬取失败: {result.get('error', '未知错误')}")
