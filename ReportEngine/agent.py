...
class FileCountBaseline:
    ...
    def check_new_files(self, directories: Dict[str, str]) -> Dict[str, Any]:
        ...
        return {
            ...
            "missing_engines": [
                engine for engine, count in new_files_found.items() if count == 0
            ],
        }
...
class ReportAgent:
    ...
    def _initialize_nodes(self):
        ...
        self.template_selection_node = TemplateSelectionNode(
            self.llm_client, self.config.TEMPLATE_DIR
        )
    ...
    def generate_report(
        ...
        logger.info(
            f"输入数据 - 报告数量: {len(reports)}, 论坛日志长度: {len(forum_logs)}"
        )

        try:
            # Step 1: 模板选择
            template_result = self._select_template(
                query, reports, forum_logs, custom_template
            )

            # Step 2: 直接生成HTML报告
            html_report = self._generate_html_report(
                query, reports, forum_logs, template_result
            )
    ...
    def _select_template(
        self, query: str, reports: List[Any], forum_logs: str, custom_template: str
    ):
        """选择报告模板"""
        logger.info("选择报告模板...")
...
"""
    ...
    def check_input_files(
        self, insight_dir: str, media_dir: str, query_dir: str, forum_log_path: str
    ) -> Dict[str, Any]:
        """
        ...
        for engine, new_count in check_result["new_files_found"].items():
            ...
            if new_count > 0:
                ...
                result["missing_files"].append(
                    f"{engine}: {current_count}个文件 (基准{baseline_count}个，无新增)"
                )
    ...
    def check_input_files_selective(
        self,
        enabled_directories: Dict[str, str],
        forum_log_path: str,
        enabled_engines: Dict[str, bool],
    ) -> Dict[str, Any]:
        """
        检查启用的引擎输入文件是否准备就绪（基于文件数量增加）

        Args:
            enabled_directories: 启用的引擎报告目录字典
            forum_log_path: 论坛日志文件路径
            enabled_engines: 启用的引擎配置

        Returns:
            检查结果字典
        """
        # 使用文件基准管理器检查新文件（只检查启用的引擎）
        check_result = self.file_baseline.check_new_files(enabled_directories)
        ...
        # 获取最新文件路径（用于实际报告生成）
        if result["ready"]:
            result["latest_files"] = self.file_baseline.get_latest_files(
                enabled_directories
            )
            if forum_ready:
                result["latest_files"]["forum"] = forum_log_path
