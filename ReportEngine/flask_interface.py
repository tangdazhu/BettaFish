...
def get_enabled_engines() -> Dict[str, bool]:
    """从.env文件读取启用的引擎配置"""
    try:
        # 查找.env文件
        ...
        if env_file_path.exists():
            ...
            for line in env_content.splitlines():
                ...
                if line.startswith("ENABLED_ENGINES="):
                    ...
                    try:
                        enabled_engines = json.loads(value)
                    except:
                        pass
                    break
        ...
        logger.warning(f"读取启用引擎配置失败: {e}，使用默认配置")
        return {"insight": True, "media": True, "query": True}
...
def check_engines_ready() -> Dict[str, Any]:
    ...
    logger.info(f"当前启用的引擎: {enabled_engines}")
    ...
    # 调用agent的检查方法，只传入启用的引擎目录
    return report_agent.check_input_files_selective(
        enabled_directories, forum_log_path, enabled_engines
    )
        ...
        return jsonify(
            {
                ...
                "enabled_engines": engines_status.get("enabled_engines", {}),
...
def get_progress(task_id: str):
    ...
    try:
        ...
        return jsonify({"success": True, "task": current_task.to_dict()})
...
def get_templates():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "templates": templates, "template_dir": template_dir}
        )
...
def get_report_log():
    ...
    try:
        ...
        if not os.path.exists(log_file):
            return jsonify({"success": True, "log_lines": []})
